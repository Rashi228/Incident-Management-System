...
def generate_with_gemini(prompt: str) -> str:
    logger.debug("generate_with_gemini: STARTING REST CALL")
    try:
        ...
        logger.debug("generate_with_gemini: Sending HTTPX POST to Gemini API")
        ...
        data = response.json()
        logger.debug("generate_with_gemini: Successfully got response from Gemini REST API")
        ...
        logger.error(f"generate_with_gemini: CAUGHT EXCEPTION: {e}")
...
def generate_pdf(title: str, subtitle: str, content: str, report_type: str) -> bytes:
    ...
    try:
        ...
        logger.debug(f"multi_cell failed on title: {e}")
        pass
    ...
    for line in content.split("\n"):
        ...
        if stripped and (stripped[0].isdigit() and stripped[1] in ".):") or stripped.isupper():
            ...
            try:
                ...
                logger.debug(f"multi_cell failed on header line: {e}")
                pass
            pdf.set_font("Helvetica", "", 11)
            pdf.set_text_color(30, 41, 59)
        else:
            try:
                ...
                logger.debug(f"multi_cell failed on body line: {e}")
                pass
...
def generate_and_upload_report(
    ...
    logger.debug("generate_and_upload_report: Calling generate_with_gemini")
    content = generate_with_gemini(prompt)
    logger.debug("generate_and_upload_report: generate_with_gemini complete. Calling generate_pdf")
    pdf_bytes = generate_pdf(pdf_title, pdf_subtitle, content, report_type)
    logger.debug("generate_and_upload_report: generate_pdf complete. Calling upload_pdf_to_supabase")
    url = upload_pdf_to_supabase(pdf_bytes, file_name)
    logger.debug(f"generate_and_upload_report: upload_pdf_to_supabase complete. URL: {url}")
    return url
